...
if [ -z $UPSTREAM ]; then
    ...
    ansible all -i $HOME/inventory.yml -a 'yum-config-manager --disable \*'
    ansible all -i $HOME/inventory.yml -a 'subscription-manager repos --enable="rhel-7-server-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ansible-2.6-rpms"'
