...
if [ -e $HOME/certificates_vars.yml ]; then
  EXTRA_VARS="-e='@$HOME/certificates_vars.yml'"
fi
...
ansible-playbook -i $HOME/inventory.yml $EXTRA_VARS /usr/share/ansible/openshift-ansible/playbooks/prerequisites.yml -vvv | tee ansible-$(date +%Y%m%d%H%M%S)-prerequisites.log
ansible-playbook -i $HOME/inventory.yml $EXTRA_VARS /usr/share/ansible/openshift-ansible/playbooks/deploy_cluster.yml -vvv | tee ansible-$(date +%Y%m%d%H%M%S)-deploy_cluster.log
